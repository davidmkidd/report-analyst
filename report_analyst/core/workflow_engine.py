    ...
    async def execute(self, context: WorkflowContext) -> WorkflowContext:
        ...
        try:
            ...
            if use_llm_scoring and context.llm_scores:
                ...
                sorted_chunks = sorted(context.chunks, key=lambda x: x.get('similarity_score', 0.0), reverse=True)
...
class EvidenceAssignmentStep(WorkflowStep):
    ...
    def validate_input(self, context: WorkflowContext) -> bool:
        return (context.chunks is not None and 
                context.evidence_chunks is not None and 
                context.analysis_result is not None)
