...
class CacheManager:
    ...
    def init_db(self):
        ...
        try:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_file_path ON document_chunks(file_path)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chunk_params ON document_chunks(chunk_size, chunk_overlap)")
    ...
    async def get_similar_chunks(self, query_embedding: np.ndarray, file_path: str, top_k: int = 5, chunk_size: int = None, chunk_overlap: int = None) -> List[Dict]:
        ...
        try:
            ...
            for node in nodes:
                ...
                similarity_score = node.score if hasattr(node, 'score') else node.get_score() if hasattr(node, 'get_score') else 0.0
            ...
            if chunks:
                logger.info(f"Similarity score range: {min(c['score'] for c in chunks):.4f} - {max(c['score'] for c in chunks):.4f}")
    ...
    def get_document_chunks(self, file_path: str, chunk_size: int = None, chunk_overlap: int = None) -> List[Dict]:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                chunks = []
                for row in rows:
                    chunk_id, chunk_text, embedding_bytes, metadata_json, chunk_size, chunk_overlap = row
